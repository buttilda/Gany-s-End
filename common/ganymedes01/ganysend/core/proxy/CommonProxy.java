...
public class CommonProxy implements IGuiHandler {
	...
	public Object getServerGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
		if (ID == GUIsID.BASIC_FILTERING_HOPPER) {
			TileEntityFilteringHopper tileBasicHopper = (TileEntityFilteringHopper) world.getBlockTileEntity(x, y, z);
			return new ContainerFilteringHopper(player.inventory, tileBasicHopper);
		}
		if (ID == GUIsID.ADVANCED_FILTERING_HOPPER) {
			TileEntityAdvancedFilteringHopper tileAdvancedHopper = (TileEntityAdvancedFilteringHopper) world.getBlockTileEntity(x, y, z);
