...
public class BlockTimeManipulatorRender implements ISimpleBlockRenderingHandler {
	...
	public boolean renderWorldBlock(IBlockAccess blockAccess, int x, int y, int z, Block block, int modelId, RenderBlocks renderer) {
		...
		IIcon front = block.getIcon(frontID, 0);
		IIcon left = block.getIcon(leftID, 0);
		IIcon right = block.getIcon(rightID, 0);
		IIcon back = block.getIcon(backID, 0);
		if (meta < 4)
	...
	private void renderTop(RenderBlocks renderer, Block block, int x, int y, int z, IIcon front, IIcon left, IIcon right, IIcon back) {
		float pixel = 1.0F / 16.0F;
		...
		IIcon surface = block.getIcon(0, 1);
		renderer.renderFaceYPos(block, x, y, z, surface);
		renderer.renderFaceYNeg(block, x, y, z, surface);
	}

	private void renderBottom(RenderBlocks renderer, Block block, int x, int y, int z, IIcon front, IIcon left, IIcon right, IIcon back) {
		float pixel = 1.0F / 16.0F;
