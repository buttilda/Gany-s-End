...
public class EnderTogglerAir extends Block implements IConfigurable {
	...
	public void randomDisplayTick(World world, BlockPos pos, IBlockState state, Random rand) {
		int max = 5;
		for (int j = 1; j <= max; j++) {
			double d0 = pos.getX() + j * 0.20D;
			double d1 = pos.getY();
			double d2 = pos.getZ() + 0.5D;
			world.spawnParticle(EnumParticleTypes.PORTAL, d0, d1, d2, 0.0D, 0.0D, 0.0D);
		}
		for (int j = 1; j <= max; j++) {
			double d0 = pos.getX() + 0.5D;
			double d1 = pos.getY();
			double d2 = pos.getZ() + j * 0.20D;
			world.spawnParticle(EnumParticleTypes.PORTAL, d0, d1, d2, 0.0D, 0.0D, 0.0D);
		}
	}
	...
	public void onNeighborBlockChange(World world, BlockPos pos, IBlockState state, Block neighbour) {
		if (world.isBlockIndirectlyGettingPowered(pos) <= 0)
			world.setBlockState(pos, ModBlocks.ender_toggler.getDefaultState(), 2);
	}
